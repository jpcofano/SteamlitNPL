...
# Descargar los recursos necesarios para la lematización
nltk.download('averaged_perceptron_tagger')
nltk.download('wordnet')

# Crear instancias de lematizador y stemmer
lemmatizer = WordNetLemmatizer()
stemmer = PorterStemmer()

# Función para realizar la lematización de un texto
def lemmatize_text(text):
    tokens = nltk.word_tokenize(text)  # Tokenizar el texto en palabras
    tagged_tokens = nltk.pos_tag(tokens)  # Realizar POS tagging
    lemmatized_tokens = []
    
    for token, tag in tagged_tokens:
        # Filtrar sustantivos, verbos, adjetivos y adverbios para lematizar
        if tag.startswith('N') or tag.startswith('V') or tag.startswith('J') or tag.startswith('R'):
            lemma = lemmatizer.lemmatize(token, get_wordnet_pos(tag))  # Lematizar palabra con su categoría gramatical
            lemmatized_tokens.append(lemma)
        else:
            lemmatized_tokens.append(token)
    
    lemmatized_text = ' '.join(lemmatized_tokens)  # Unir las palabras lematizadas en un texto
    return lemmatized_text

# Función para realizar el stemming de un texto
def stem_text(text):
    tokens = nltk.word_tokenize(text)  # Tokenizar el texto en palabras
    tagged_tokens = nltk.pos_tag(tokens)  # Realizar POS tagging
    stemmed_tokens = []
    ...
    for token, tag in tagged_tokens:
        # Filtrar sustantivos, verbos, adjetivos y adverbios para stemmizar
        if tag.startswith('N') or tag.startswith('V') or tag.startswith('J') or tag.startswith('R'):
            stemmed = stemmer.stem(token)  # Realizar stemming en la palabra
            stemmed_tokens.append(stemmed)
        else:
            stemmed_tokens.append(token)
    
    stemmed_text = ' '.join(stemmed_tokens)  # Unir las palabras stemmizadas en un texto
    return stemmed_text

# Función para mapear las etiquetas POS de NLTK a las categorías gramaticales de WordNet
def get_wordnet_pos(tag):
    if tag.startswith('N'):
        ...
        return wordnet.NOUN  # Por defecto, utilizar sustantivo como categoría gramatical
...
if st.button("Generar"):
    if cloud_type == "wordcloud":
        stop_words = get_stopwords()
        min_word_length=3
        text_input = stem_text(text_input)
